            ...
            .route(rota -> rota
                ...
                .filters(filtro -> filtro.rewritePath("/microcustomers/(?<segment>.*)","/${segment}")
                    ...
                    .circuitBreaker(config -> config.setName("microcustomersCircuitBreaker") // Pode usar qualquer nome para o Circuit Breaker
                        ...
                        .metadata(CONNECT_TIMEOUT_ATTR, 2000) // Tempo máximo que o Gateway espera para estabelecer uma conexão com o serviço de destino
                        .metadata(RESPONSE_TIMEOUT_ATTR, 200000) // Tempo máximo que o Gateway espera para receber uma resposta do serviço de destino após a conexão ser estabelecida
            ...
            .route(rota ->
                ...
                .filters(filtro -> filtro.rewritePath("/microempresas/(?<segment>.*)", "/${segment}")
                    ...
                    .circuitBreaker(config -> config.setName("microempresasCircuitBreaker")
                        ...
                        .metadata(RESPONSE_TIMEOUT_ATTR, 200000)
